...
const AddCollector = ({ walletData, setWalletData}) => {
    ...
    return (
        <div className="form-control w-3/6 flex flex-col items-center ">
         ...
         <input type="text" placeholder="Public Key" value={wallet} onChange={(e) => setWallet(e.target.value)} className="input input-bordered"/>
         <button onClick = {()=>{
             setWalletData([...walletData, {name:name, wallet:wallet}])
             setName("");
             setWallet("");
         }}
         className="btn btn-outline my-10 bg-sky-400 max-w-xs w-2/6">Add Wallet</button>
        </div>
    )
}
